...
- Progress:
  ...
  - 2026-04-26 milestone 23 started: investigate and fix the real Web-streaming-but-Feishu-card-stale incident. Evidence for Feishu message `om_x100b51ea6b5738b0b3968d3483ea4e6`: lifecycle stopped at `stream_started`, `router_state.active_streaming_turns` still tracked the Feishu route, Web snapshot `web:main` contained live text, and no `finalized` / `im_delivered` / `cursor_committed` rows existed yet.
  - 2026-04-26 milestone 23: Feishu streaming cards now create from `idle` for pre-answer auxiliary progress (`tool_use_start`, status, hook, todo), so long Codex tool/status phases are visible in Feishu instead of only Web. Validation passed with `npm test -- --run tests/feishu-streaming-card.test.ts`, `npm test -- --run tests/stream-presentation.test.ts tests/feishu-e2e.test.ts`, `npm run typecheck`, `git diff --check`, `npx prettier --check src/feishu-streaming-card.ts tests/feishu-streaming-card.test.ts PLANS/ACTIVE.md PLANS/ROADMAP.md`, and `./scripts/review.sh`.
  ...
  - 2026-04-26 milestone 25 applied: commit `357f756 Gate interrupted context resume`; safe restart `restart-2026-04-26T11-34-00-685Z-5b68e5d4` passed, `/api/health` returned healthy for backend PID `14317`, and the post-restart process snapshot showed one backend plus one current runner group with no historical orphan runner group visible.
- Iteration plan:
...
- Progress:
  ...
  - 2026-04-26 milestone 21: `make start` and the default LaunchAgent install path now route through the canonical `cli-claw start` launcher contract instead of direct backend commands. Validation passed with `npm test -- --run tests/launch-command-contract.test.ts`, `npm run typecheck`, `git diff --check`, and `./scripts/review.sh`.
- Next action:
  - Continue launch command contract hardening: decide whether to rename or relabel `package.json` `npm start`, then make build-staleness reporting source-aware.
