...
Cli Claw 是一个单实例自托管的 CLI Agent 平台。它不重新实现 Agent 内核，而是把成熟的 CLI Agent 运行时封装成统一服务，让你通过 Web、飞书和微信入口访问同一套工作区、文件、workflow 和流式执行能力。
...
- 文件与 workflow：工作区文件管理、workflow 定时计划和工作流审计统一接入。
...
- `cli-claw start` 会把“你启动命令时所在的目录”当作主工作区默认执行目录，并在缺失时物化到 `custom_cwd`。
...
- 主工作区的默认执行 / 文件根目录来自 `cli-claw start` 的启动目录。
