...
<p align="center">
  <a href="#cli-claw-是什么">介绍</a> · <a href="#核心能力">核心能力</a> · <a href="#快速开始">快速开始</a> · <a href="#技术架构">技术架构</a> · <a href="#贡献">贡献</a>
</p>
...
Cli Claw 是一个自托管、多用户的 AI Agent 平台。它不重新实现 Agent 内核，而是把成熟的 **CLI Agent 运行时** 封装成统一服务，让你通过飞书、Telegram、QQ 和 Web 界面访问同一套工作区、文件、任务和流式执行能力。

当前已接入两类运行时：

- **Claude Runtime**：`Claude Agent SDK + Claude Code CLI`
- **Codex Runtime**：`Codex CLI + codex-acp`

核心设计原则：**不重复造轮子，直接复用成熟 CLI Agent**。主进程只负责多用户隔离、消息路由、队列调度、持久化、Web/IM 体验与运行时编排；真正的推理、工具调用和会话循环交给底层 CLI runtime。

### 关键特性

- **多运行时执行层** — Claude Runtime（Claude Agent SDK + Claude Code CLI）与 Codex Runtime（Codex CLI + codex-acp）统一接入
...
- **运行时身份可观测** — 每条助手回复都可携带 agent / model / effort / token / cost 元数据，便于排障和计费归因

> 项目借鉴了 [OpenClaw](https://github.com/nicepkg/OpenClaw) 的容器化架构，也吸收了 Claude Code 官方 [Cowork](https://github.com/anthropics/claude-code/tree/main/packages/cowork) 的多会话协作思路：多个独立 Agent 会话并行工作，各自拥有隔离工作空间和持久记忆，结果通过 IM 渠道送达。
...
Cli Claw 的执行层不绑定单一 SDK。主进程只负责选择运行时与执行模式，真正的 Agent 会话由 `agent-runner` 调度到底层 CLI runtime：

| Agent 类型 | 底层运行时 | 支持执行模式 | 认证方式 |
| ---------- | ---------- | ------------ | -------- |
| `claude`   | Claude Agent SDK + Claude Code CLI | `host` / `container` | Web 向导配置 Claude Provider（OAuth / setup-token / API Key） |
...
- **宿主机模式** — Agent 直接在宿主机运行，访问本地文件系统，支持 Claude / Codex（admin 主工作区默认模式）
- **容器模式** — Docker 隔离执行，非 root 用户，当前用于 Claude Runtime（member 主工作区默认模式）
...
- **至少准备一种运行时认证方式**
  - Claude Runtime：在 Web 设置向导中配置 Claude Provider（OAuth / setup-token / API Key）
...
> Claude Runtime 无需手动额外安装 `claude` CLI。项目依赖的 Claude Agent SDK 已内置对应 CLI 运行时，`make start` 首次启动时会自动安装。
> Codex Runtime 依赖宿主机上的全局 Codex CLI 登录态，因此需要先执行一次 `codex login`。
...
2. **配置默认 Claude Provider** — 填入 API 密钥 / OAuth / setup-token（支持中转服务）
...
>
> 如需使用 Codex：在服务器执行 `codex login` 后，到工作区运行时设置里把 Agent 切换为 `Codex`。
...
4. 发布应用版本并等待审批通过
5. 在 cli-claw Web 界面的「设置 → IM 通道 → 飞书」中填入 App ID 和 App Secret
...
2. 记录返回的 Bot Token
3. 在 cli-claw Web 界面的「设置 → IM 通道 → Telegram」中填入 Bot Token
...
4. 在 cli-claw Web 界面的「设置 → IM 通道 → QQ」中填入 App ID 和 App Secret
...
| 模式           | 说明                                          | 支持运行时              | 适用对象                                  | 前置要求                  |
| -------------- | --------------------------------------------- | ----------------------- | ----------------------------------------- | ------------------------- |
| **宿主机模式** | Agent 直接在宿主机运行，访问本地文件系统      | Claude / Codex          | admin 主工作区（`folder=main`）           | Node.js；Codex 需 `login` |
| **容器模式**   | Agent 在 Docker 容器中隔离运行，预装 40+ 工具 | Claude                  | member 主工作区（`folder=home-{userId}`） | Docker + 构建镜像         |

admin 主工作区默认使用宿主机模式，member 注册后自动创建容器模式的主工作区。也可在 Web 界面的会话管理中手动切换执行模式；若选择 Codex，系统会自动约束到宿主机模式。

### 容器工具链

当前容器镜像主要服务于 **Claude Runtime**。Codex 走宿主机执行，不依赖容器镜像。
...
flowchart TD
    ...
    subgraph 执行层
        Host["宿主机进程<br/>(agent-runner / host)"]
        Container["Docker 容器<br/>(agent-runner)"]
    end

    subgraph Agent["CLI Agent 运行时"]
        Claude["Claude Runtime<br/>(Claude Agent SDK + Claude Code CLI)"]
        Codex["Codex Runtime<br/>(Codex CLI + codex-acp)"]
    ...
    Memory["记忆系统<br/>(AGENTS.md + memory/)"]
    ...
    class Claude,Codex,MCP,Stream faas
    class IPC cfg
    class Memory cfg
```

**数据流**：消息从接入层进入主进程，经去重和路由后分发到并发队列。队列启动宿主机进程或 Docker 容器，内部的 `agent-runner` 再根据工作区的 `agentType` / `executionMode` 选择 Claude Runtime 或 Codex Runtime。Claude 路径通过 Claude Agent SDK 驱动完整 CLI 会话，Codex 路径通过 Codex CLI + ACP 建立流式回路。流式事件（思考、文本、工具调用等）通过 stdout 标记协议传回主进程，再经 WebSocket 广播到 Web 客户端或通过 IM API 回复到飞书 / Telegram。MCP Server 通过基于文件的 IPC 通道提供工具，实现 Agent 与主进程的双向通信。
...
| **Agent**   | Claude Agent SDK · Claude Code CLI · Codex CLI · codex-acp · MCP SDK · IPC 文件通道                                                                        |
...
│   ├── sessions/{folder}/.claude/#   Claude Runtime 会话持久化
...
- `~/.claude/.credentials.json`：Claude Runtime 的本地登录态来源之一
- `~/.codex/config.toml`：Codex Runtime 的模型 / effort 配置
- `codex login`：Codex Runtime 的宿主机登录态
...
以下为可选覆盖项。推荐使用 Web 设置向导配置默认 Claude Provider 和 IM 凭据（加密存储）；Codex 相关认证通过宿主机 `codex login` 处理。

| 变量                            | 默认值                  | 说明                                |
| ------------------------------- | ----------------------- | ----------------------------------- |
| `WEB_PORT`                      | `3000`                  | Web 服务端口                        |
| `ASSISTANT_NAME`                | `cli-claw`              | 助手显示名称                        |
| `CONTAINER_IMAGE`               | `cli-claw-agent:latest` | Agent 容器镜像                      |
