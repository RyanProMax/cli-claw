...
- Workflow run 是另一类独立执行上下文：用户会话只触发 workflow 并接收进度/最终结果；workflow 自身按 `(folder, workflowId)` 生成内部 `workflowContextId` / LangGraph `thread_id`，并使用独立 runtime session。workflow state、structured artifacts、checkpoint、step output 不能写入用户会话主 runtime session；LangGraph checkpoint 落在独立 SQLite 文件 `~/.cli-claw/db/workflow-checkpoints.sqlite`。
