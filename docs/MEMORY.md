...
- 如果新消息前方存在未消费的 `interrupt_partial` 残留，Cli Claw 不再回放旧中断上下文；只处理当前未提交的用户消息。即使存在旧的 pending resume confirmation，用户回复“继续上次”也只作为当前消息送入 runtime，由底层 runtime session 自己决定是否能续上。
