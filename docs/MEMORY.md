...
> 本文负责：Cli Claw 的消息数据库、runtime session 和上下文边界。工作区 / 线程 / 入口路由身份见 `docs/ARCHITECTURE.md`；运行时 session 与 workspace cwd 见 `docs/RUNTIME.md`。
...
- `.agents/workflows/*.json` 是仓库级 workflow 定义，`.agents/agent-roles/*.md` 是 workflow runtime role card，`.agents/roles/*.md` 是仓库执行协议角色定义，`.agents/skills/**/SKILL.md` 是仓库内联 skill 定义，`~/.agents/agents/*.md` 是用户级 Agent 定义；这些文件都不是 Cli Claw 消息历史注入来源。工作区 workflow/role card 优先，缺失时可以使用 Cli Claw 包内置 workflow/role card。
