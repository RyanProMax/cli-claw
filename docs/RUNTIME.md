...
- Codex ACP 原始流必须按事件类型先分类：`agent_thought_chunk` 进入 `thinking_delta` / `thinkingText`；`agent_message_chunk` 进入 `text_delta`。明显的进度前缀（例如 `我会先...`、`我继续...`、`Context compacted`、测试/验证/回归状态）即使仍由 `agent_message_chunk` 承载，也必须进入 `commentaryText` / `Thinking`，不能先直播到正文。非进度正文按同一 `messageUuid` 累积为当前 `answerText` / Web `partialText`；当新的 Codex assistant `messageUuid` 到来时，上一条未终态确认的 assistant message 才降级累积到 `commentaryText` / `Thinking`，当前 message 继续作为正文候选。Feishu/Web 渲染层必须把 `thinkingText` 与 `commentaryText` 合并为同一个 `Thinking` 辅助区，不再渲染单独“过程”折叠栏。terminal raw/final output 仍必须经过可见性分类后写入完成态正文；若 terminal raw/final 与 streaming `commentaryText` 是同一份纯过程文本，完成态必须保留到 `Thinking`，正文使用空完成态兜底，不能把同一份过程文本展示为正文。若 terminal raw/final 由多段进度叙述拼接最终完成总结（例如 `已完成并提交...`、`本次做了...`）组成，必须把前缀剥离进 `Thinking`，只从完成总结开始发送正文。若这份 terminal raw/final 还包含强结构化最终正文边界（例如 `**/research｜...**` 或 `**港股 IPO 池｜...**`），即使前缀超过普通过程长度限制，也必须剥离前缀进 `Thinking`，并从该标题开始发送正文。
- Feishu card 必须把 tool `steps` 作为顶部第一个执行轨迹面板展示，`Thinking` 紧随其后，再渲染 status/hook/todo 和最终正文；完成态也不能把 steps 折叠栏移到正文底部。Codex commentary/process 文本进入 `Thinking`，长中文过程按句子边界拆行，避免堆成一整段。若 Codex terminal final 把“我会先…”这类过程句粘在 `**/research｜...**`、`**港股 IPO 池｜...**` 等报告标题前，`reply-visibility` 必须把这段剥离出正文，并同步进顶部 `Thinking` 区域。
