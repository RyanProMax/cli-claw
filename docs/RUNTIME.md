...
股票策略自迭代采用状态驱动调度，而不是固定 30 分钟完整 discovery。启动迁移会创建 `web:stock-strategy` / `stock-strategy` 工作区，并把 `stock-strategy-*` scheduled task 迁入该工作区；原飞书/微信目标保存在 `notify_channels`。planner 终态必须给 scheduler 保留固定 JSON schema：`action`、`next_workflow`、`cadence`、`reason`、`evidence_signature`、`requires_human`、`strategy_usability`。`strategy_usability` 使用 `stock_strategy_usability_v1`，结构为 `status`、`standard_version`、`passed_checks`、`failed_checks`、`missing_checks`、`summary`。scheduler 读取该 JSON 后可执行 `continue`、`pause` / `pause_discovery`、`slow_down`、`switch_workflow` 和 `ask_human`：降频当前任务，按 cadence 创建/更新下游 workflow，或只在 `requires_human=true` / `ask_human` 时向外部 IM 通道投递。真正暂停受可用标准门控：只有 `strategy_usability.status=passed` 时才允许把任务置为 `paused`；未通过或未知时，`pause` / `pause_discovery` 会被降级为 active 的 cooldown / slow_down，并继续进入验证、设计复盘或覆盖检查。`evidence_signature` 采用 `market:factor:universe:cost_model:holding_window:data_version`，连续相同且无新增候选或字段时应短路为 cooldown / 降频，而不是重复完整 Agent review。

股票策略状态机按市场维护：`coverage_check -> discovery -> candidate_review -> candidate_validation -> human_review_ready -> approved/rejected/cooldown`。当前内置下游 workflow 是：`stock-strategy-us-candidate-validation` 用 `stock.strategy.candidate_validation` 补 OOS 分段、`momentum_5d` vs `momentum_20d` champion/challenger、行业/主题集中度、`average_amount_5d` / `turnover_rate`、回撤、换手、成本敏感性和更可解释 universe；`stock-strategy-hk-design-review` 用 `stock.strategy.design_review` 做 forward window、成本、universe 覆盖和 blocked 原因复盘；`stock-strategy-cn-coverage-check` 用 `stock.strategy.coverage_check` 判断 CN universe seed / scan 是否仍为空。`stock_strategy_usability_v1` 的可用标准包括：mandatory artifacts 完整且非 degraded、OOS 分段不依赖单一近期切片、候选与 champion 在相同 universe / holding window / cost model 下比较、流动性字段 `average_amount_5d` 和 `turnover_rate` 可读、回撤/换手/成本敏感性不打没净 edge、universe 不停留在 5 个标的小样本且集中度可解释、人工审批边界仍然存在。`stock-strategy-loop` 仍用于成熟/候选复盘，不能承担前期空转发现。所有股票策略 workflow 都不能伪造实盘收益或策略价值；discovery 默认不写 registry，显式 `recordToRegistry=true` 也只允许记录候选/评估/proposal，不允许 approve / activate。股票策略最终 `plan_next_iteration` 若遇到可识别的 OpenAI 临时过载、socket 断开或 5xx，会基于已完成 artifact 生成 `status=degraded` 的只读 fallback plan。
