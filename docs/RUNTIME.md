...
- Streaming card 将 tool `steps`、单一 `Thinking` 辅助区和主正文分区渲染。主正文只承载 answer；Codex 的 `text_delta` presentation 流不是主正文权威来源。
...
- Codex `text_delta` 是 presentation/process 流，不是 answer 流；共享 presentation state 必须把它累积到 `commentaryText`，不能写入 `answerText`、Web `partialText` 或恢复 buffer 的正文字段。Feishu/Web 渲染层必须把 `thinkingText` 与 `commentaryText` 合并为同一个 `Thinking` 辅助区，不再渲染单独“过程”折叠栏。主正文必须等 terminal raw/final output 经过可见性分类后写入，避免 presentation 过渡文本覆盖当前 turn 的最终答案；若 terminal raw/final 与 streaming `commentaryText` 是同一份纯过程文本，完成态必须保留到 `Thinking`，正文使用空完成态兜底，不能把同一份过程文本展示为正文。若这份 terminal raw/final 还包含强结构化最终正文边界（例如 `**/research｜...**` 或 `**港股 IPO 池｜...**`），即使前缀超过普通过程长度限制，也必须剥离前缀进 `Thinking`，并从该标题开始发送正文。
