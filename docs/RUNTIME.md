...
股票策略自迭代采用主控心跳 + 动态 worker 调度，而不是固定 30 分钟完整 discovery。启动迁移会创建 `web:stock-strategy` / `stock-strategy` 工作区，并把 `stock-strategy-*` scheduled task 迁入该工作区；原飞书/微信目标保存在 `notify_channels`。默认只保留 `stock-strategy-control-loop` 的 30 分钟主控心跳和 `stock-strategy-daily-progress-summary` 的每日总结，旧的固定 discovery / validation / design / coverage / paper worker schedule 会被暂停，等待主控按状态重新唤醒。主控终态必须给 scheduler 保留固定 JSON schema：`action`、`next_workflow`、`cadence`、`reason`、`evidence_signature`、`requires_human`、`strategy_usability`，并优先输出动态字段 `current_next_run_at`、`next_workflows[]`、`quality_gate`、`work_budget`。`action` 只能是 `continue`、`pause`、`pause_discovery`、`slow_down`、`switch_workflow`、`ask_human`；新的 `dispatch_*` 之类动作会让本轮 task run 直接记为 error，不再静默忽略。`next_workflows[]` 每项包含 `workflow_id`、`next_run_at`、`cadence`、`priority`、`reason`、`prompt` 和可选 `quality_gate`；scheduler 会创建或更新对应 scheduled workflow，`next_run_at="immediate"` 表示马上进入队列，ISO 时间表示精确下一次运行，若 Agent 给出已经过去的时间则回退到当前 cadence 计算的未来时间。旧字段 `current_cadence` / `next_cadence` 仍兼容，但不再代表策略推导周期。
