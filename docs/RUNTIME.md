...
股票策略自迭代采用主控心跳 + 动态 worker 调度，而不是固定 30 分钟完整 discovery。启动迁移会创建 `web:stock-strategy` / `stock-strategy` 工作区，并把 `stock-strategy-*` scheduled task 迁入该工作区；原飞书/微信目标保存在 `notify_channels`。默认只保留 `stock-strategy-control-loop` 的 30 分钟主控心跳和 `stock-strategy-daily-progress-summary` 的本地 21:00 每日总结，旧的固定 discovery / validation / design / coverage / paper worker schedule 会被暂停，等待主控按状态重新唤醒。主控终态必须给 scheduler 保留固定 JSON schema：`action`、`next_workflow`、`cadence`、`reason`、`evidence_signature`、`requires_human`、`strategy_usability`，并优先输出动态字段 `current_next_run_at`、`next_workflows[]`、`quality_gate`、`work_budget`。`action` 只能是 `continue`、`pause`、`pause_discovery`、`slow_down`、`switch_workflow`、`ask_human`；新的 `dispatch_*` 之类动作会让本轮 task run 直接记为 error，不再静默忽略。`next_workflows[]` 每项包含 `workflow_id`、`next_run_at`、`cadence`、`priority`、`reason`、`prompt` 和可选 `quality_gate`；scheduler 只会创建已知 workflow，旧别名 `stock-strategy-design-review` 会归一到 `stock-strategy-hk-design-review`，未知 `stock-strategy-*` id 会被忽略，不会变成空转 scheduled task。`next_run_at="immediate"` 表示马上进入队列，ISO 时间表示精确下一次运行，若 Agent 给出已经过去的时间则回退到当前 cadence 计算的未来时间。旧字段 `current_cadence` / `next_cadence` 仍兼容，但不再代表策略推导周期。

股票策略状态机按市场维护：`coverage_check -> discovery -> candidate_review -> candidate_validation -> human_review_ready -> approved/rejected/cooldown`。主控 Agent 负责状态、证据签名、优先级、预算和派工；阶段 worker 负责 `stock-strategy-discovery-loop`、`stock-strategy-us-candidate-validation`、`stock-strategy-hk-design-review`、`stock-strategy-cn-coverage-check`、`stock-strategy-paper-setup`、`stock-strategy-paper-validation` 等只读产物；质量 Agent 输出 `stock_strategy_quality_gate_v1`，结构为 `status`、`standard_version`、`stage`、`score`、`passed_checks`、`failed_checks`、`missing_checks`、`defects`、`summary`。`strategy_usability.status=passed` 只是策略可用标准，真正暂停或进入人审还要求 `quality_gate.status=passed`；质量门缺失或失败时，scheduler 会阻止 pause，并把任务保持 active cooldown、补证或重新派 worker。`evidence_signature` 采用 `market:factor:universe:cost_model:holding_window:data_version`，连续相同且无新增候选或字段时只记录无新增证据，不重复完整 Agent review。

`stock-strategy-us-candidate-validation` 用 `stock.strategy.candidate_validation` 补 OOS 分段、`momentum_5d` vs `momentum_20d` champion/challenger、行业/主题集中度、`average_amount_5d` / `turnover_rate`、回撤、换手、成本敏感性和更可解释 universe；`stock-strategy-hk-design-review` 做 forward window、成本、universe 覆盖和 blocked 原因复盘；`stock-strategy-cn-coverage-check` 判断 CN universe seed / scan 是否仍为空；`stock-strategy-paper-setup` 先检查 watch 列表、paper ledger、候选映射、费用/滑点、成交字段和对账入口是否具备；`stock-strategy-paper-validation` 再读取 paper/live ledger、paper/live reconciliation、OOS backtest、费用、滑点、成交和资金占用证据，只有通过质量门才允许进入 `human_review_ready` / `ask_human`，仍禁止 Agent 自动 approve、activate 或真实下单。`stock-strategy-daily-progress-summary` 每天本地 21:00 汇总关键进展，最多 8 行，只写今日完成、当前状态、阻塞项、下一步和人工需求。所有股票策略 workflow 都不能伪造实盘收益或策略价值；discovery 默认不写 registry，显式 `recordToRegistry=true` 也只允许记录候选/评估/proposal，不允许 approve / activate。

股票策略 workflow 的用户可见终态消息会在 command 层做只影响展示的摘要化；若 planner 输出 fixed scheduler decision，命令层会保留精简 `[Scheduler Decision]` JSON 给 scheduler 读取，包括 `current_next_run_at`、`next_workflows`、`quality_gate`、`work_budget` 等动态字段，而不把完整 `candidate_tasks` / `change_summary` 长 JSON 回投到 IM。scheduled task 在发送到 Web / 飞书 / 微信前会再剥离 `[Scheduler Decision]` 和调度 JSON；内部原文仍保留在 `workflow_runs.result`、`workflow_run_steps.output` 与 `task_run_logs.result`。股票策略对外 `notify_channels` 默认静默：worker 的普通成功、失败、无新增证据、scheduler decision 缺失和 runtime 堆栈只回到 Web / 审计；飞书/微信只收每日 21:00 进度总结，或 `requires_human=true` / `ask_human` 的短结论。若每日总结 workflow 自身失败，飞书/微信收到的是不含堆栈的短 fallback，原始错误只留在 Web 与 task run log。Web workflow 看板会从 planner decision、quality gate、local task artifact 和 scheduled task 聚合 US/HK/CN 状态、证据签名、下游 workflow、cadence、下一次主控运行和人工确认需求；原始 role JSON / 长文本仍保留在 workflow run 和 step 审计表中，避免为了可读性丢失排查证据。
