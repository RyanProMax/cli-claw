...
`stock-strategy-loop` 是内置 scheduled workflow 示例：`stock.strategy.collect_results` 只读 stock-analysis-api task-chain SQLite，输出最近 tasks、pending/running、summaries 和 handoff outputs；`stock.strategy.analyze_value` 调用 stock-analysis-api 内部 CLI 生成 paper/live ledger 摘要与 HK/US alpha daily report / backtest summary。任一底层 CLI 失败时只返回 degraded 子 artifact，后续角色必须明确数据缺口，不能伪造实盘收益或策略价值。
