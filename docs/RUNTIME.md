...
股票策略自迭代采用状态驱动调度，而不是固定 30 分钟完整 discovery。启动迁移会创建 `web:stock-strategy` / `stock-strategy` 工作区，并把 `stock-strategy-*` scheduled task 迁入该工作区；原飞书/微信目标保存在 `notify_channels`。planner 终态必须给 scheduler 保留固定 JSON schema：`action`、`next_workflow`、`cadence`、`current_cadence`、`next_cadence`、`reason`、`evidence_signature`、`requires_human`、`strategy_usability`。`strategy_usability` 使用 `stock_strategy_usability_v1`，结构为 `status`、`standard_version`、`passed_checks`、`failed_checks`、`missing_checks`、`summary`。scheduler 读取该 JSON 后可执行 `continue`、`pause` / `pause_discovery`、`slow_down`、`switch_workflow` 和 `ask_human`：`current_cadence` 控制当前 scheduled task，`next_cadence` 控制下游 workflow；旧字段 `cadence` 只作为下游 cadence fallback 或非 discovery 任务当前 cadence。`stock-strategy-discovery-loop` 默认保持 30 分钟轻量 orchestrator，连续相同 evidence signature 只短路完整 Agent / discovery review，不再把 orchestrator 降到 2h / 6h。真正暂停受可用标准门控：只有 `strategy_usability.status=passed` 时才允许把任务置为 `paused`；未通过或未知时，`pause` / `pause_discovery` 会被降级为 active 的 cooldown / slow_down，并继续进入验证、设计复盘或覆盖检查。`evidence_signature` 采用 `market:factor:universe:cost_model:holding_window:data_version`，连续相同且无新增候选或字段时应短路为无新增证据记录，而不是重复完整 Agent review。

股票策略状态机按市场维护：`coverage_check -> discovery -> candidate_review -> candidate_validation -> human_review_ready -> approved/rejected/cooldown`。当前内置下游 workflow 是：`stock-strategy-us-candidate-validation` 用 `stock.strategy.candidate_validation` 补 OOS 分段、`momentum_5d` vs `momentum_20d` champion/challenger、行业/主题集中度、`average_amount_5d` / `turnover_rate`、回撤、换手、成本敏感性和更可解释 universe，默认 2 小时节奏；`stock-strategy-hk-design-review` 用 `stock.strategy.design_review` 做 forward window、成本、universe 覆盖和 blocked 原因复盘，默认 6 小时或事件触发；`stock-strategy-cn-coverage-check` 用 `stock.strategy.coverage_check` 判断 CN universe seed / scan 是否仍为空，覆盖坏时默认 1 小时巡检，只有连续确认长期无源时才允许降到 6 小时；`stock-strategy-daily-progress-summary` 每 24 小时汇总发现、回测/OOS、模拟盘 / paper ledger、阻塞项、下一步节奏和人工需求。`stock_strategy_usability_v1` 的可用标准包括：mandatory artifacts 完整且非 degraded、OOS 分段不依赖单一近期切片、候选与 champion 在相同 universe / holding window / cost model 下比较、流动性字段 `average_amount_5d` 和 `turnover_rate` 可读、回撤/换手/成本敏感性不打没净 edge、universe 不停留在 5 个标的小样本且集中度可解释、人工审批边界仍然存在。`stock-strategy-loop` 仍用于成熟/候选复盘，不能承担前期空转发现。所有股票策略 workflow 都不能伪造实盘收益或策略价值；discovery 默认不写 registry，显式 `recordToRegistry=true` 也只允许记录候选/评估/proposal，不允许 approve / activate。股票策略最终 `plan_next_iteration` 若遇到可识别的 OpenAI 临时过载、socket 断开或 5xx，会基于已完成 artifact 生成 `status=degraded` 的只读 fallback plan。

`stock-strategy-discovery-loop` / `stock-strategy-loop` 的用户可见终态消息会在 command 层做只影响展示的摘要化；若 planner 输出固定 scheduler decision，命令层会保留精简 `[Scheduler Decision]` JSON 给 scheduler 读取，而不把完整 `candidate_tasks` / `change_summary` 长 JSON 回投到 IM。Web workflow 看板会从 planner decision、local task artifact 和 scheduled task 聚合 US/HK/CN 状态、证据签名、下游 workflow、cadence 和人工确认需求；原始 role JSON / 长文本仍保留在 workflow run 和 step 审计表中，避免为了可读性丢失排查证据。
