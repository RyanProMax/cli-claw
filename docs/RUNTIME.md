...
- Codex `text_delta` 是 presentation/process 流，不是 answer 流；共享 presentation state 必须把它累积到 `commentaryText`，不能写入 `answerText`、Web `partialText` 或恢复 buffer 的正文字段。Feishu 可把它直播到独立“过程”折叠面板，但主正文必须等 terminal raw/final output 到达后一次性写入，避免 presentation 过渡文本覆盖当前 turn 的最终答案。
