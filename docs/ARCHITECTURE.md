...
- `container/agent-runner/`：执行引擎，负责根据 `agentType` 选择 Claude Runtime 或 Codex Runtime，并处理流式事件、MCP 工具、上下文压缩和后台任务通知。
...
4. `agent-runner` 根据工作区运行时配置选择 Claude Runtime 或 Codex Runtime，产生文本、思考、工具调用和任务事件。
