...
股票策略自分析 / 自迭代现在使用同一 workflow 编排层，而不是独立的分钟级 launchd tick 直接驱动 Agent。系统启动时会幂等创建 `web:stock-strategy`（显示名“股票策略”、folder `stock-strategy`）工作区，并把 `stock-strategy-*` scheduled workflow 迁入该工作区；历史飞书/微信入口只作为 `notify_channels` 保留，不再把股票策略跑在主工作区。固定 30 分钟不再是策略挖掘周期，只是 `stock-strategy-control-loop` 的主控心跳：主控 Agent 先读取新数据、新证据、质量门、候选成熟度、paper/live ledger 和人工反馈，再用 JSON 输出 `current_next_run_at`、`next_workflows[]`、`quality_gate` 与 `work_budget`。scheduler 根据这些字段动态设置当前任务下次运行时间、创建或唤醒多个 worker workflow，并只在 `ask_human` / `requires_human=true` 时外推飞书/微信；若 `action` 不是白名单里的 `continue`、`pause`、`pause_discovery`、`slow_down`、`switch_workflow`、`ask_human`，本轮 task run 会直接记 error，防止派工被静默忽略。`stock-strategy-discovery-loop`、US 验证、HK 设计复盘、CN 覆盖检查、`stock-strategy-paper-setup` 和 `stock-strategy-paper-validation` 都是 worker，不再默认按固定间隔空跑；旧固定 worker schedule 会在启动迁移中暂停，等待主控按状态重新派工。质量 Agent 通过 `stock_strategy_quality_gate_v1` 独立验收 OOS、champion/challenger、流动性、成本、回撤、换手、样本解释性、simulated trading 准备度和 paper/live reconciliation；若质量门未通过，即使 planner 认为可暂停，scheduler 也会阻止进入真正暂停或人工审批。每个市场维护 `coverage_check -> discovery -> candidate_review -> candidate_validation -> human_review_ready -> approved/rejected/cooldown` 状态。scheduler 对超时或进程重启遗留的 running task log / workflow run 做 watchdog 清理，避免主控或日报永久停在 running。所有 workflow 都只产出审阅和计划 evidence，不自动 approve、不 activate、不触发 broker。
