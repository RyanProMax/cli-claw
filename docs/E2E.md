...
- live smoke 会真实发送 `[e2e] ...` 消息。用户已授权默认直接执行真实飞书 E2E；优先使用当前可发现的飞书私聊入口或测试入口，不再为发送 smoke 消息单独确认。
- 用户明确说“当前入口测试”“没有群”时，可使用当前私聊入口，不要反复要求新凭据或测试群。
- 若只有生产群聊入口，先确认是否允许发 smoke 消息；默认不要往生产群发测试消息。
