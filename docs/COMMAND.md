...
`scheduled_tasks.execution_type='workflow'` 时，`script_command` 保存 workflow id，`prompt` 保存 workflow prompt；scheduler 会复用 `/workflow <id> <prompt>` 的同一条执行路径，并写 workflow run/step 审计。股票策略 scheduled workflow 归属独立 `web:stock-strategy` 工作区；`stock-strategy-discovery-loop` 不再无条件 30 分钟完整 discovery，而是读取 planner 固定 JSON 决策并执行 pause / slow_down / switch_workflow / ask_human。US 候选验证、HK 设计复盘、CN 覆盖检查分别由 `stock-strategy-us-candidate-validation`、`stock-strategy-hk-design-review`、`stock-strategy-cn-coverage-check` 承接；成熟或候选复盘仍使用 `stock-strategy-loop`。所有股票策略 workflow 都不自动 approve、不 activate、不触发 broker。
...
- 内置 `stock-strategy-discovery-loop` workflow 用于前期策略路由：先收集 stock-analysis-api task-chain 状态，再运行 `stock.strategy.discovery_cycle` 形成 discovery evidence；随后 readonly reviewer / planner 判断是否有新数据、新证据或设计变化。planner 必须输出固定 JSON schema（`action`、`next_workflow`、`cadence`、`reason`、`evidence_signature`、`requires_human`），scheduler 据此暂停、降频、触发下游 workflow 或请求人工。默认不写 registry；只有显式 workflow input `recordToRegistry=true` 时才记录候选/评估/proposal，仍禁止 approve / activate。
- 内置 `stock-strategy-us-candidate-validation` workflow 用于 US 候选验证：`stock.strategy.candidate_validation` 补 OOS 分段、`momentum_5d` vs `momentum_20d` champion/challenger、行业/主题集中度、`average_amount_5d` / `turnover_rate`、回撤、换手、成本敏感性和更可解释 universe，只有证据站住才进入 `human_review_ready`。
...
- 内置 `stock-strategy-cn-coverage-check` workflow 用于 CN 覆盖检查：`stock.strategy.coverage_check` 只判断 universe seed / scan 是否仍为空；`scanned=0` 时保持覆盖修复，不进入空 discovery。
...
- 股票策略 workflow 投递到飞书/微信时不会原样输出长 JSON。命令层会把普通终态压缩成四块短摘要：`🎯 阶段目标`、`📍 本轮完成`、`📈 策略效果`、`🧭 后续规划`；若最终包含 fixed scheduler decision，则保留精简 `[Scheduler Decision]` JSON 给 scheduler 读取。外部 `notify_channels` 只在 `requires_human=true` 或 `ask_human` 时收到结论，重复无新增的 pause / slow_down / switch_workflow 只回到股票策略 Web 工作区；完整结果仍保留在 `workflow_runs.result` 和 `workflow_run_steps.output` 供审计。
...
Web 导航包含 `自动化` 页，内部按 `计划`、`运行`、`工作流` 三个视角整合定时任务管理、当前运行状态和 workflow 审计。`计划` 复用 `scheduled_tasks` 管理能力；`运行` 聚合当前 running / queued 状态；`工作流` 查看当天 workflow run、step 进度、定时 workflow task、任务日志和当前运行中状态。看板复用 workflow / scheduler 审计表，不触发重跑。股票策略区域会展示 US/HK/CN 的 `discovering` / `validating` / `blocked` / `cooldown` / `human_review_ready` 状态、证据签名、下游 workflow、cadence 和是否需要人工。看板可编辑 / 删除 `execution_type='workflow'` 的定时任务：编辑会更新 `scheduled_tasks` 中的 workflow id、prompt、调度和值状态；删除只移除后续调度任务，不强制中断已经启动的 workflow run，既有 run/step 审计继续保留。旧 `/tasks` 与 `/workflows` 入口不再保留兼容跳转，Web 自动化能力统一从 `/automations` 进入。
