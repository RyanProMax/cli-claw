...
`scheduled_tasks.execution_type='workflow'` 时，`script_command` 保存 workflow id，`prompt` 保存 workflow prompt；scheduler 会复用 `/workflow <id> <prompt>` 的同一条执行路径，并写 workflow run/step 审计。股票策略 scheduled workflow 归属独立 `web:stock-strategy` 工作区；`stock-strategy-discovery-loop` 不再无条件 30 分钟完整 discovery，而是读取 planner 固定 JSON 决策并执行 pause / slow_down / switch_workflow / ask_human。股票策略 pause 受 `strategy_usability` 门控：只有 `stock_strategy_usability_v1` 达到 `status=passed` 才能真正暂停；未通过或未知时会保持 active 并降频 / cooldown / 切下游验证。US 候选验证、HK 设计复盘、CN 覆盖检查分别由 `stock-strategy-us-candidate-validation`、`stock-strategy-hk-design-review`、`stock-strategy-cn-coverage-check` 承接；成熟或候选复盘仍使用 `stock-strategy-loop`。所有股票策略 workflow 都不自动 approve、不 activate、不触发 broker。
...
- 内置 `stock-strategy-discovery-loop` workflow 用于前期策略路由：先收集 stock-analysis-api task-chain 状态，再运行 `stock.strategy.discovery_cycle` 形成 discovery evidence；随后 readonly reviewer / planner 判断是否有新数据、新证据或设计变化。planner 必须输出固定 JSON schema（`action`、`next_workflow`、`cadence`、`reason`、`evidence_signature`、`requires_human`、`strategy_usability`），scheduler 据此降频、触发下游 workflow、请求人工，或在策略可用标准通过后暂停。默认不写 registry；只有显式 workflow input `recordToRegistry=true` 时才记录候选/评估/proposal，仍禁止 approve / activate。
