...
- 输入 bare `/workflow` 会列出当前工作区可用 workflow；输入 `/workflow <id> <任务>` 会创建一条 `workflow_runs` 审计记录并后台执行对应 graph。run 创建成功后，Web / IM 会立即收到字段化启动回执，包含 workflow id、run id、任务和回投说明；飞书入口还会收到一张独立 workflow 进度卡，用 emoji 字段和强制换行实时展示每个节点的状态、内容摘要、耗时和本地任务；成功、失败或 runner 超时后，系统会再向同一触发会话发送 `✅ 完成` 或 `❌ 失败` 终态消息。
