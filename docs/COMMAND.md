...
`scheduled_tasks.execution_type='workflow'` 时，`script_command` 保存 workflow id，`prompt` 保存 workflow prompt；scheduler 会复用 `/workflow <id> <prompt>` 的同一条执行路径，并写 workflow run/step 审计。股票策略 scheduled workflow 归属独立 `web:stock-strategy` 工作区；固定 30 分钟只保留给 `stock-strategy-control-loop` 主控心跳，不再作为 discovery 或回测周期。主控读取 planner 固定 JSON 决策并执行 pause / slow_down / switch_workflow / ask_human，同时支持动态字段 `current_next_run_at`、`next_workflows[]`、`quality_gate`、`work_budget`：`current_next_run_at` 控制主控精确下次运行，`next_workflows[]` 可创建或唤醒 US 验证、HK 设计复盘、CN 覆盖检查、discovery、paper setup 或 paper validation worker。`action` 只允许 `continue`、`pause`、`pause_discovery`、`slow_down`、`switch_workflow`、`ask_human`；不认识的动作会把本轮 task run 记为 error，不再被静默忽略。股票策略 pause 受 `strategy_usability` 与 `quality_gate` 双门控：只有策略可用标准和独立质量门都通过才允许真正暂停或进入人审；未通过或未知时会保持 active cooldown、补证或切下游 worker。下游 workflow id 必须是已知 `stock-strategy-*`；旧别名 `stock-strategy-design-review` 会归一到 `stock-strategy-hk-design-review`，未知 id 不会创建 scheduled task。每日进度由 `stock-strategy-daily-progress-summary` 在本地 21:00 输出。所有股票策略 workflow 都不自动 approve、不 activate、不触发 broker。
...
- 内置 `stock-strategy-control-loop` workflow 是股票策略唯一固定短周期主控心跳：先收集 task-chain、backtest、paper/live ledger 和最近 evidence，再由 `stock-strategy-quality-reviewer` 输出 `quality_gate`，最后由 `stock-strategy-chief-orchestrator` 输出 scheduler JSON。它必须优先使用 `current_next_run_at`、`next_workflows[]`、`quality_gate`、`work_budget` 动态安排 worker，不能把固定 30 分钟当作策略推导周期；`action` 必须来自 scheduler 白名单，不允许输出自造派工动作。
...
- 内置 `stock-strategy-paper-setup` workflow 用于模拟盘验证前的准备检查：确认 watch 列表、paper ledger、候选映射、费用/滑点、成交字段和对账入口是否齐备；齐备后再派 `stock-strategy-paper-validation`。
...
- 内置 `stock-strategy-daily-progress-summary` workflow 用于每日 21:00 进度总结：收集 task-chain summary、handoff、paper/live ledger、HK/US alpha daily report 和 backtest summary，只输出最多 8 行关键结论：今日完成、当前状态、阻塞项、下一步、是否需要人工。它只报告进度，不输出 scheduler JSON，不 approve、不 activate、不交易。
- 股票策略 workflow 投递到飞书/微信时不会原样输出长 JSON。命令层会把普通终态压缩成四块短摘要：`🎯 阶段目标`、`📍 本轮完成`、`📈 策略效果`、`🧭 后续规划`；若最终包含 fixed scheduler decision，则内部结果会保留精简 `[Scheduler Decision]` JSON 给 scheduler 读取，包含 `current_next_run_at`、`next_workflows`、`quality_gate` 和 `work_budget`。scheduled task 真正发送到 Web / 飞书 / 微信前会剥离 `[Scheduler Decision]` 与调度 JSON；外部 `notify_channels` 只在 `requires_human=true`、`ask_human` 或每日进度总结时收到结论，重复无新增、普通 worker 失败、scheduler decision 缺失和 runtime 堆栈只回到股票策略 Web 工作区；若日报 workflow 自身失败，外部只收到不含堆栈的短 fallback。完整结果仍保留在 `workflow_runs.result`、`workflow_run_steps.output` 和 `task_run_logs.result` 供审计。
