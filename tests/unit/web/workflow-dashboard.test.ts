    ...
    workflowRuns: [
      {
        ...
        context_id: 'ctx-1',
        folder: 'main',
        workflow_id: 'research-daily',
        thread_id: 'thread-1',
        trigger_chat_jid: 'web:main',
        trigger_message_id: null,
        trigger_user_id: null,
        prompt: 'Run daily research',
        status: 'running',
        result: null,
        error: null,
        metadata: {
...
describe('workflow dashboard aggregation', () => {
  ...
  test('does not expose a stock strategy dashboard section', () => {
    const dashboard = buildWorkflowDashboardData(input());

    expect('stockStrategy' in dashboard).toBe(false);
  });
