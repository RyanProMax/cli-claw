...
describe('workflow graph engine', () => {
  ...
  test('returns a degraded stock strategy plan when the final planner hits a transient runtime error', async () => {
    const workflow: WorkflowDefinition = {
      id: 'stock-strategy-discovery-loop',
      name: '股票策略短间隔发现工作流',
      description: '短间隔 discovery',
      roles: ['analyst'],
      start: 'review_snapshot',
      nodes: [
        {
          id: 'review_snapshot',
          type: 'local_task',
          taskId: 'test.discovery_review',
          outputArtifact: 'strategy_discovery_review',
        },
        {
          id: 'plan_next_iteration',
          type: 'role_task',
          roleId: 'analyst',
          outputArtifact: 'next_iteration_plan',
        },
      ] as any,
      edges: [
        { from: 'review_snapshot', to: 'plan_next_iteration' },
        { from: 'plan_next_iteration', to: '__end__' },
      ],
      maxRetries: 0,
      sourcePath:
        '/workspace/.agents/workflows/stock-strategy-discovery-loop.json',
    };
    const role = makeRole();
    const context = {
      ...makeContext(),
    ...
    const run = { ...makeRun(), workflow_id: 'stock-strategy-discovery-loop' };
    const stepEvents: unknown[] = [];
    ...
    const result = await runWorkflowGraph({
      ...
      localTasks: {
        'test.discovery_review': async () => ({
          new_candidates: [
            {
              market: 'us',
              candidate: 'alpha_topn_momentum_5d.20260521',
              summary: '候选仅可补证，不可上线。',
              evidence: { rank_ic_mean: 0.058 },
            },
          ],
          repeated_candidates: ['HK same blocked reason'],
          blocked_reasons: ['CN scanned=0'],
          recommended_checks: ['补齐 OOS 分段', '做 champion/challenger 对比'],
        }),
      },
      runner: async () => ({
        status: 'error',
        result: null,
        error:
          "Agent process exited with code 1: message: 'Our servers are currently overloaded. Please try again later.', code: 'server_is_overloaded'",
      }),
      recordStep: (step) => {
        stepEvents.push(step);
      },
    ...
    expect(parsed).toMatchObject({
      ...
      change_summary: expect.stringContaining(
        '上游 discovery review 已保留 1 个新增候选',
      ),
      repeat_decision: expect.objectContaining({
        summary: expect.stringContaining('repeated=1'),
      }),
    });
    expect(parsed.candidate_tasks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          task_name: 'alpha_topn_momentum_5d.20260521',
          market: 'us',
        }),
      ]),
    );
