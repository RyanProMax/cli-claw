...
describe('app root helpers', () => {
  test('detects installed cli-claw package roots inside node_modules', () => {
    expect(
      isInstalledNodeModulesPackageRoot(
        '/Users/ryan/.nvm/versions/node/v24.14.0/lib/node_modules/cli-claw',
      ),
    ).toBe(true);
    expect(
      isInstalledNodeModulesPackageRoot(
        '/opt/homebrew/lib/node_modules/cli-claw',
      ),
    ).toBe(true);
  });
  ...
  test('does not treat local checkouts as installed package roots', () => {
    ...
    expect(isInstalledNodeModulesPackageRoot('/tmp/cli-claw')).toBe(false);
  });
});
