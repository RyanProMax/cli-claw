...
def direction_emoji(change_pct: float) -> str:
    if change_pct > 0:
        return "📈"
    if change_pct < 0:
        return "📉"
...
def format_snapshot_row(
    *,
    ...
    prefix = direction_emoji(change_pct)
    if abs(change_pct) >= ABS_MOVE_THRESHOLD:
        prefix += "⚡"
    if reasons:
        return f"{prefix} {symbol} {name} {price_text} {pct_text}｜{'；'.join(reasons)} 🚨"
