...
export function MemoryPage() {
  ...
  const loadSources = useCallback(async () => {
    ...
    try {
      ...
      if (!nextSelected) {
        // If folder param provided, try to find matching session AGENTS.md first
        if (folderParam) {
          nextSelected =
            data.sources.find(
              (s) =>
                ...
                s.path.endsWith('AGENTS.md'),
            )?.path || null;
        }
      }

      if (!nextSelected) {
        // Default: global AGENTS.md → first session AGENTS.md → first available
        nextSelected =
          data.sources.find((s) => s.type === 'global')?.path ||
          data.sources.find(
            (s) => s.type === 'session' && s.path.endsWith('AGENTS.md'),
          )?.path ||
          data.sources[0]?.path ||
          null;
      }
